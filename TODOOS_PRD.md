...
### 2.2 Categories
모든 투두는 카테고리를 가지며, 카테고리별 고유 **색상(Color)**으로 스택 내에서 명확히 구분되어야 한다.

**[기본 카테고리]**
1.  **일상 (Daily)**: 기본값.
2.  **자산 관리 (Finance)**: 가계부, 결제 등.
3.  **운동 (Workout - Health OS Linked)**:
    *   Health OS와 데이터 연동.
    *   Health OS의 "오늘의 운동"이 완료되면 Todo OS에서도 자동으로 Done 스택으로 이동(또는 그 반대)하는 양방향 동기화 지원.

**[확장성]**
*   이 3가지 기본 카테고리 외에 사용자가 자유롭게 새로운 카테고리를 추가/수정/삭제할 수 있다.
